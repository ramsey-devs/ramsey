...
class ARMA(dist.Distribution):
    ...
    def __init__(self, loc, ar_coefficients, ma_coefficients, scale, length=None):
        super().__init__()
        self.loc = loc
    ...
    def sample(self, rng_key, length=None, initial_state=None, sample_shape=()):
        ...
        def body_fn(states, errors, sample_key):
            ...
            loc += jnp.einsum("i,i->", lhs, rhs)

            errors = jnp.atleast_1d(errors)
            take = jnp.minimum(self.q, errors.shape[0])
            lhs = jax.lax.dynamic_slice_in_dim(
                errors, errors.shape[0] - take, take
            )[::-1]
            rhs = jax.lax.dynamic_slice_in_dim(self.ma_coefficients, 0, take)
            loc += jnp.einsum("i,i->", lhs, rhs)
